(ns ^{:clojure.tools.namespace.repl/load false
  ...
  "Replaces clear-reply / refresh-reply with parallel impls"
  (:require
    ...
    (let [{:keys [exec]} (meta session)]
      (exec id
            (fn []
              (try
                (before-reply msg)
                (reloading-reply clojure.tools.namespace.repl/refresh-tracker msg)
                (binding [*ns* (create-ns 'user)] ;; work around ex thrown by in-ns call. Use `user` to possibly avoid weird errors
                ...
                (let [tracker-after clojure.tools.namespace.repl/refresh-tracker]
                  (result-reply tracker-after msg)
                  (after-reply tracker-after msg))
...
(def clear-reply
  (fn [{:keys [transport session id] :as msg}]
    (let [{:keys [exec]} (meta session)]
      (exec id
            (fn []
