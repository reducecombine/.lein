(ns clostache.parser
  "https://github.com/fhd/clostache/issues/63"
...
(defn- render-section
  ...
  (let [section-data ((keyword (:name section)) data)]
    (if (:inverted section)
      (if (or (and (clojure.core.incubator/seqable? section-data) (empty? section-data))
              (not section-data))
        (:body section))
      (if section-data
        (if (fn? section-data)
          ...
          (let [section-data (cond (sequential? section-data)                     section-data
                                   ...
                                   (clojure.core.incubator/seqable? section-data) (seq section-data)
                                   :else                                          [{}])
