  ...
  (let [members
        (for [^Class class classes
              ...
              :when (and (not (static? member))
                         (not (-> member .getName (.contains "$$YJP$$"))))]
...
(defn try-get-object-class
  ...
  [ns context]
  (when (or (= (:idx (first context)) 0)
            (= '. ;; patched
               (some-> context first :form first)))
    (let [form (second (:form (first context)))
          primary-bindings (set (bindings-from-context context))
          let-thing (some->> context
                             reverse ;; get the largest ctxs first (full threading, surrounding let, etc)
                             (some (fn [{:keys [form]}]
                                     (and (seq? form)
                                          (every? map? form)
                                          (->> form
                                               (filter (fn [{:keys [form]}]
                                                         (and (seq? form)
                                                              (-> form first name symbol #{'let}))))
                                               first)))))
          fallback-bindings (some->> let-thing
                                     vector
                                     bindings-from-context
                                     set)
          binding (get primary-bindings form (get fallback-bindings form))
          bound-value (try
                        (some-> let-thing
                                :form
                                compliment.sources.local-bindings/extract-local-binding-mappings
                                (get form))
                        (catch Exception _
                          nil))]
      (if-let [tag (or
                    ...
                    (try
                      (when (instance? clojure.lang.IObj bound-value)
                        (some-> bound-value meta :tag process-tag))
                      (catch Throwable e
                        nil))

                    (try
                      (when (symbol? bound-value)
                        (some->> bound-value (ns-resolve ns) meta :tag process-tag))
                      (catch Throwable e
                        nil)))]
        ;; We have a tag - try to resolve the class from it.
        (resolve-class ns tag)
        ;; Otherwise, try to resolve symbol to a Var.
        (or (let [obj (and (symbol? form) (ns-resolve ns form))]
              (and (= (class obj) clojure.lang.Var)
                   (class (deref obj))))
            (when (and (seq? form)
                       (-> form first ctor-sym?))
              (->> form first ctor-sym->class-sym (resolve-class ns)))
            (when (and bound-value
                       (seq? bound-value)
                       (some-> bound-value first ctor-sym?))
              (->> bound-value first ctor-sym->class-sym (resolve-class ns))))))))
...
(defn populate-static-members-cache
  ...
  [^Class class]
  (loop [cache {}, [^Member c & r] (remove (fn [member]
                                             (-> member .getName (.contains "$$YJP$$")))
                                           (concat (.getMethods class)
                                                   (.getFields class)))]
    (if c
      (if (static? c)
